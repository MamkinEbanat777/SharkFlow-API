...
export const logTaskUpdate = (title, changes, userUuid, ip) =>
  logInfo('Task', 'updated', `"${title}" by user ${userUuid} from IP: ${ip}, changes: ${JSON.stringify(changes)}`);

export const logTaskFetch = (tasksCount, totalTasks, userUuid, ip) => {
  if (process.env.NODE_ENV === 'development') {
    logInfo('Task', 'fetched', `${tasksCount}/${totalTasks} by user ${userUuid} from IP: ${ip}`);
  }
};
