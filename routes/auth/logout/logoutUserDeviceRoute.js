...
router.post('/api/auth/logout/:deviceId', authenticateMiddleware, async (req, res) => {
  ...
  if (currentDeviceId && deviceId === currentDeviceId) {
    return res.status(400).json({ error: 'Нельзя выйти с текущего устройства этим способом' });
  }
  ...
  try {
    ...
    if (!session) {
      return res.status(404).json({ error: 'Сессия не найдена или уже неактивна' });
    }
